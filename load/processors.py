...
class PlotBarCode:
    ...
    def plot_barcode(self, pieces, artist, song, *args, **kwargs):
        ...
        base_folder = os.path.dirname(pieces[0]) + "/csv_files/"
        output_folder = '/'.join(os.path.dirname(pieces[0]).split("/")[:4])
        output_folder = output_folder + "/summary/bar_codes/{}/{}/".format(artist, song)
        ...
        for idx, piece in enumerate(pieces):                                    
            ...
            for note in data.note.unique():
                out_file2 = output_folder_piece + pattern(note)
        ...
        for note in whole_data.note.unique():
            out_file = output_folder + pattern(note)
            fig, ax = plt.subplots()
